...
impl List7 {
    ...
    fn build_line(&self, width: usize, idx: usize) -> Line {
        ...
        if width > 4 + 7 {
            spans.push(Span::from(" ".repeat(width - 4 - 7)));
        }

        let value_text = format!("{:0>7b}", value);
        if self.is_selected && idx == self.cursor.1 {
            for (ch_idx, ch) in value_text.chars().enumerate() {
                let style = if 6 - ch_idx as u8 == self.cursor.0 {
...
impl CompositeWidget for List7 {
    fn update(&mut self, key: KeyEvent) {
        if key.code == KeyCode::Left && self.cursor.0 < 6 {
            self.cursor.0 += 1;
...
impl Component for List7 {
    ...
    fn insert_zero(&mut self) {
        let operand = 0xff ^ (1 << self.cursor.0);
        self.values[self.cursor.1] &= operand;
        if self.cursor.0 > 0 {
            self.cursor.0 -= 1;
        }
